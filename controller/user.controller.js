const { signupService, findByEmail, findByUserEmail, findByUserEmailToken } = require("../services/user.service");
...
exports.getMeEmail = async (req, res, next) => {
    try {
        const token = req.params.token;
        console.log(token);
        const user = await findByUserEmailToken(token);
        console.log(user);
        res.status(2000).json({
            status: 'Successfully user',
            data: user
